"""
LLM Configuration for the Moderator Agent.
...
# Model parameters - using higher temperature for more decisive verdict
DEFAULT_PARAMS = {
    "temperature": 0.5,  # Lower temperature for more factual analysis
    "top_p": 0.9,
    "max_tokens": 1000,  # Allow more tokens for comprehensive verdict
    "stop": ["</response>"]
}
...
def generate_response(prompt: str, params: Optional[Dict[str, Any]] = None) -> str:
    ...
    try:
        ...
        response = httpx.post(
            OLLAMA_API_URL,
            json=payload,
            timeout=90.0  # Longer timeout for moderator's thorough analysis
        )
