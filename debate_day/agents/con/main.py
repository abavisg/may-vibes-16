...
This agent participates in debates on the CON side, using the MCP server
...
from debate_day.agents.con.strategy import build_prompt, parse_response
from debate_day.agents.con.llm_config import generate_response

# Create logs directory at project root
project_root = Path(__file__).parent.parent.parent.parent # This assumes agents/con/main.py, so ../../../ is root
...
logger.remove()
logger.add(
    logs_dir / "con_agent_{time}.log", # Updated path
    rotation="10 MB",
    level="INFO",
...
def main():
    ...
    logger.info(f"Starting CON agent '{AGENT_NAME}' for debate {DEBATE_ID}")
    ...
    while True:
        try:
            ...
            if turn_info["debate_status"] == "finished" or turn_info["debate_status"] == "error":
                logger.info(f"Debate status is '{turn_info["debate_status"]}'. CON agent '{AGENT_NAME}' stopping.")
                break
